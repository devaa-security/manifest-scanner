...
export default class Scan extends Command {
  ...
  static flags = {
    ...
    file: Flags.string({char: 'f', description: 'Path to the Android Project'}),
    // flag with no value (-r, --report)
    report: Flags.string({char: 'r', description: 'Report format (json, text)'}),
  }
  ...
  public async run(): Promise<void> {
    ...
    if (flags.file && flags.report) {
      this.log(`you input --report and --file: ${flags.file} and ${flags.report}`)
    }
    ...
    const filePath = this.findFileInDirectory(flags.file, "AndroidManifest.xml");

    if (filePath) {
        console.log(`Found file at: ${filePath}`); 
        this.parseXmlFileToJson(filePath).then((result: any) => {
          console.log(JSON.stringify(result, null, 2));
          console.log(result.manifest.application)
        }).catch((error: any) => {});
    } else {
        this.error('AndroidManifest.xml not found. Please provide a valid path to the Android Project')
    }
